...
- [ ] **DB-01**: Prisma schema defines all 7 MVP entities (IncomeSource, Transaction, Category, Debt, Budget, Period, MonthlySummary)
- [ ] **DB-02**: All monetary fields stored as BigInt (centavos), interest rates as Int (basis points)
- [ ] **DB-03**: Enums defined: TransactionType, Frequency, DebtType, PaymentMethod, CategoryType
- [ ] **DB-04**: Indexes on Transaction(periodId, date), Transaction(categoryId), Budget(periodId, categoryId) unique, Period(month, year) unique
...
- [ ] **DB-09**: v2 entities (ValueUnit, UnitRate, Asset) included in schema but no UI/API
