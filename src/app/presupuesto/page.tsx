...
export default async function PresupuestoPage({ searchParams }: PageProps) {
  ...
  let budgets = await getBudgetsWithSpent(period.id)

  // Auto-copy: if no budgets and period is open, copy from previous
  if (budgets.length === 0 && !period.isClosed) {
    const copied = await copyBudgetsFromPreviousPeriod(period.id, period.month, period.year)
    if (copied) {
      budgets = await getBudgetsWithSpent(period.id)
    }
  }
